...
- 静的な水面光（`.water-light`、全レイアウト共通）
- モバイル限定の水紋・コピー成功・サムネイル浮上
- モバイル限定の検索一致ハイライト・スクロール水位線・読み込みスケルトン
...
- Service Workerは同一オリジンのアプリシェルのみをキャッシュし、R2の`songs.json`は対象外
...
- 楽曲の分類・検索・並び替え・URL正規化は`src/domain/songCatalog.js`の純粋関数へ集約する
- `src/app.js`はDOMイベントと各モジュールの調停を担当し、楽曲判定規則を重複実装しない

### 8.3 データ取得

- 候補URL配列を優先順で試行
- 各候補の取得は15秒で中断し、ネットワークエラー・404・408・429・5xxでは次候補を試行
- 取得後はETagの有無に依存せずキャッシュ保持（TTL管理）
- `dataVersion` が同一の場合は不要な再描画を省略
- 通信失敗時も検証済みキャッシュがあれば表示を継続
- JSONは配列形式または`items`配列を持つスキーマバージョン1のみ受理
...
- カスタム弾幕のUnicode正規化・文字列生成・TTLキャッシュは`src/features/danmaku.js`へ集約する

### 8.6 モーション制御

- スクロール量・速度から泡エフェクト強度を計算
- スクロール・リサイズ由来のレイアウト計測は`requestAnimationFrame`単位に抑制
- 泡のDOM要素は最大90個とし、fine pointer環境では生成しない
